...
public class DriverRadioactivity extends DriverEquipment {
    ...
    //MQTT Message Topic 
    String Topic_Status=this.topis.iLabRadioactivitySimulationRaw+this.topis.Status;
    
    String Topic_Validation=this.topis.iLabRadioactivitySimulationRaw+this.topis.ValidationReport;
    //String Topic_Specification=this.topis.iLabRadioactivitySimulation+this.topis.ExperimentSpecification;
    String Topic_ActExecutionTime=this.topis.iLabRadioactivitySimulationRaw+this.topis.ActExecutionTime;
    String Topic_EstExecutionTime=this.topis.iLabRadioactivitySimulationRaw+this.topis.EstExecutionTime;
    String Topic_SetAbsorberLocation=this.topis.iLabRadioactivitySimulationRaw+this.topis.SetAbsorberLocation;
    String Topic_SetSourceLocation=this.topis.iLabRadioactivitySimulationRaw+this.topis.SetSourceLocation;
    String Topic_SetTubeDistance=this.topis.iLabRadioactivitySimulationRaw+this.topis.SetTubeDistance;
    String Topic_RemainingTime=this.topis.iLabRadioactivitySimulationRaw+this.topis.RemainingTime;
    String Topic_DataCapture=this.topis.iLabRadioactivitySimulationRaw+this.topis.CaptureCounts;
   ...
   private JSonStatus JSonStatus;
    private JSonDataCapture JSonDataCapture;
    //
    ...
    public DriverRadioactivity(LabEquipmentConfiguration labEquipmentConfiguration) throws Exception {
	    ...
	    String clientId = "DRiverRadioactivity";
            boolean quietMode=false;                 
            MQTTClient = new MQTTClient(url,clientId,quietMode);
            
            //JSon objects     
            this.JSonStatus=new JSonStatus();
            this.JSonDataCapture=new JSonDataCapture();
    }
    ...
    public Validation Validate(String xmlSpecification) throws Exception {
        ...
        //publish Status
        JSonStatus.setStatus(methodName);
        this.MQTTClient.publish(this.Topic_Status, qos, JSonStatus.getJsonString().getBytes());
    ...
    protected boolean ExecuteStarting() {
        ...
        Logfile.WriteCalled(logLevel, STR_ClassName, methodName);
       System.out.println("!!!!!!!!!!!!!ExecuteStarting2222!!!!!!!!!!!!!!!!!!");
       System.out.println("topic: "+this.Topic_Status);
        ...
        try {
            //publish Status: ExecuteStarting
            JSonStatus.setStatus(StatusNames.ExecuteStarting);
            this.MQTTClient.publish(this.Topic_Status, qos, JSonStatus.getJsonString().getBytes());
            /*
            ...
            while (thisState != States.Done) {
                switch (thisState) {
                    ...
                    case SelectAbsorberMessageLine1:
                        ...
                        ///set Status { status : STRLCD_SelectAbsorber }
                        JSonStatus.setStatus(StatusNames.SelectAbsorber);
                    ...
                    case SelectAbsorberMessageLine2:
                       ...
                       ///set Status { detail : absorberName }
                        JSonStatus.setDetail(absorberName);
                        
                        thisState = States.SelectAbsorber;
                        break;

                    case SelectAbsorber:
                        //publish status : SelectAbsorber
                        this.MQTTClient.publish(this.Topic_Status, qos, JSonStatus.getJsonString().getBytes());
                    ...
                    case SelectSourceMessageLine1:
                        ...
                        ///set Status { status : STRLCD_SelectSource }
                        JSonStatus.setStatus(StatusNames.SelectSource);
                    ...
                    case SelectSourceMessageLine2:
                        ...
                        ///set Status { detail : sourceName }
                        JSonStatus.setDetail(sourceName);
                    ...
                    case SelectSource:
                        ...
                        this.MQTTClient.publish(Topic_Status, qos, JSonStatus.getJsonString().getBytes());
                ...
                if (this.cancelled == true) {
                   ...
                   //pulbish Status
                   JSonStatus.setStatus(Cancelled);
                   this.MQTTClient.publish(Topic_Status, qos, JSonStatus.getJsonString().getBytes());
            ...
            while (thisState != States.Done) {
                switch (thisState) {
                    ...
                    case SetTubeDistanceMessageLine1:
                        ...
                        //set Status {status : STRLCD_SetDistance }
                        this.JSonStatus.setStatus(StatusNames.SetTube);
                    ...
                    case SetTubeDistanceMessageLine2:
                        ...
                        if (this.deviceSerialLcd.WriteLine(DeviceSerialLcd.LineNumber.Two, lcdMessage) == false) {
                         ...
                         //set detail {detail : lcdMessage }
                        this.JSonStatus.setDetail(lcdMessage);
                    ...
                    case SetTubeDistance:
                        //publish MQTT status
                        System.out.println("!!!set tube has sent!!!");
                        this.MQTTClient.publish(Topic_Status, qos, JSonStatus.getJsonString().getBytes());
                    ...
                    case CaptureDataMessageLine1:
                        ...
                        //set Status {status : STRLCD_CaptureCounts }
                        this.JSonStatus.setStatus(StatusNames.CaptureCounts);
                    ...
                    case CaptureData:
                        ...
                        //publish MQTT JSonDataCapture
                        JSonDataCapture.SetData(data);
                        JSonDataCapture.SetdistanceIndex(distanceIndex);
                        JSonDataCapture.SettrialsIndex(repeatIndex);
                        this.MQTTClient.publish(Topic_DataCapture, qos, JSonDataCapture.getJsonString().getBytes());
                ...
                if (this.cancelled == true) {
                   ...
                   JSonStatus.setStatus(StatusNames.Cancelled);
                   this.MQTTClient.publish(Topic_DataCapture, qos, JSonDataCapture.getJsonString().getBytes());
    ...
    protected boolean ExecuteStopping() {
        ...
        try {
            //publish status ExecuteComplete
            JSonStatus.setStatus(StatusNames.ExecuteComplete);
            this.MQTTClient.publish(Topic_Status, qos, JSonStatus.getJsonString().getBytes());
            ...
            while (thisState != States.Done) {
                switch (thisState) {
                    ...
                    case ReturnSourceMessageLine1:
                        ...
                        //set status {status : STRLCD_ReturnSource}
                        JSonStatus.setStatus(StatusNames.SelectSource);
                    ...
                    case ReturnAbsorberMessageLine1:
                        ...
                        //set status {status : SelectAbsorber}
                        JSonStatus.setStatus(StatusNames.SelectAbsorber);
                    ...
                    case ReturnTubeMessageLine1:
                        ...
                        //set status {status : STRLCD_ReturnTube}
                        JSonStatus.setStatus(StatusNames.SetTube);
                    ...
                    case ReturnToReadyMessageLine1:
                        ...
                        JSonStatus.setStatus(StatusNames.Ready);
